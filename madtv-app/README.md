...
Ordner-Build (`publicDir` ist für die Einzeldatei abgeschaltet). Die Weiche
stand dabei lange auf `MODE === 'single'` — die Vollbild-Fassung läuft aber
unter `vollbild` und forderte beides an, obwohl neben einer einzelnen Datei
nichts liegt. Jetzt zählen beide Einzeldatei-Fassungen gleich.

Fürs Ablegen auf dem Startbildschirm sagt das Manifest `display: standalone`.
Safari liest es dafür nicht — es braucht `apple-mobile-web-app-capable`, und
das Meta-Feld fehlte. Sonst läge
