...
import {
  ATTR_OFF, NEWS_QUAL, POP, SLOTS, adSlotOf, isPrime, slotHour, slotLabel,
} from './constants';
import { getDay, reachOf, toast, sfx, emit, trendOf, startOf } from './state';
import { viewers } from './format';
import type { BlockResult, Channel, Game, Licence, Slot } from './types';
...
/** Attraktivität eines Halbstundenfeldes je Zielgruppe. */
export function blockAttraction(
  g: Game, ch: Channel, day: number, slot: number, newsAttr: number[],
): number[] {
  const slots = getDay(ch, day);
  const s = slots[slot]!;
  const out = new Array(GROUPS.length).fill(0.05);
  const hour = slotHour(slot);
  // Die Nachrichten laufen zu Beginn jeder Stunde; in der zweiten Hälfte
  // wirken sie nur noch halb so stark nach.
  const newsW = slot % 2 === 0 ? 0.3 : 0.15;

  if (!s.prog) {
    for (let i = 0; i < GROUPS.length; i++) out[i] = 0.05 + newsAttr[i]! * (newsW / 3);
    return out;
  }
  ...
  const begin = startOf(slots, slot);

  // Trailer: lief das Programm heute schon auf einem Werbeplatz?
  let trailer = 0;
  for (let b = 0; b < begin; b++) {
    if (slots[b]!.trailer?.uid === p.uid) trailer += 0.13;
  }

  // Zuschauerfluss aus dem Feld vor dem Sendungsbeginn
  let flow = 0;
  const prev = begin > 0 ? slots[begin - 1]!.prog : null;
  if (prev && prev.uid !== p.uid) {
  ...
  let loyal = 0;
  if (p.isSerie && p.lastBlock !== null) loyal = p.lastBlock === begin ? 0.13 : -0.09;

  // Je länger die Sendung läuft, desto mehr Zuschauer steigen aus. Ohne diesen
  // Abfall wäre der Dreistünder immer die beste Wahl.
  const fatigue = Math.pow(0.955, slot - begin);

  const star = ch.star && ch.star.genres.includes(p.genre) ? ch.star.boost : 0;
  const tr = trendOf(g, p.genre);

  for (let i = 0; i < GROUPS.length; i++) {
    let a = (qEff / 52) * gd.aff[i]! * tr * fatigue;
    a *= 1 + newsAttr[i]! * newsW;
...
/** Zuschauer aller Sender für ein Feld, ohne Nebenwirkungen. */
...
/** Ein Halbstundenfeld tatsächlich ausstrahlen — mit allen Folgen. */
export function airBlock(g: Game, day: number, block: number): void {
  ...
  g.ch.forEach((ch, ci) => {
    ...
    if (s.prog) {
      const p = s.prog;
      // Abnutzung, Folgenzähler und Kulturpunkte gelten der Sendung, nicht
      // dem einzelnen Feld — sonst verschliesse ein Dreistünder sich sechsfach.
      if (s.start) {
        p.aired++;
        p.lastDay = day;
        p.lastBlock = block;
      ...
      if (GENRES[p.genre].kult) {
        ...
        ch.culturePoints += (p.critic / 100) * (r.total / 1_000_000) * starK * 0.5;
        ch.cultureToday +=
          ((p.critic / 100) * (1 + r.total / 6_000_000) + (p.bettyBonus ?? 0) * 0.16) * starK * 0.5;
      }
  ...
  const own = getDay(g.player, day)[block]!;
  // Nur der Sendungsbeginn kommt ins Protokoll, sonst stünde ein Dreistünder
  // sechsmal darin
  if (own.start || !own.prog) {
    g.log.unshift({
      day, block,
      aud: pr.total,
      share: pr.total / all,
      title: own.prog?.title ?? 'Testbild',
    });
...
/** Alle Felder eines Tages nachholen, die noch nicht liefen. */
...
/** Zuschauer einer ganzen Sendung: Mittel über ihre Felder. */
export function programmeAudience(slots: Slot[], at: number): number {
  const p = slots[at]?.prog;
  if (!p) return slots[at]?.res?.total ?? 0;
  let sum = 0;
  let n = 0;
  slots.forEach((s) => {
    if (s.prog?.uid === p.uid && s.res) { sum += s.res.total; n++; }
  });
  return n ? sum / n : 0;
}

export { adSlotOf, slotLabel };
